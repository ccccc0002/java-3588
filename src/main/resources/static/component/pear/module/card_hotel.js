layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
	...
	var card_hotel = function(opt) {
		_instances[opt.elem.substring(1)] = this;
		this.reload(opt);
	};
	card_hotel.prototype.initOptions = function (opt) {
	...
	card_hotel.prototype.init = function () {
		...
		if (!!url) {
			...
			getData(url).then(function(data){
				...
				if (!!option.data && option.data.length > 0) {
					...
					html = "<p style='text-align: center; font-size: 16px; font-weight: bold; padding: 150px;'>没有数据</p>";
				}
				$(option.elem).html(html);
				if (option.page) {
					laypage.render({
	...
	card_hotel.prototype.reload = function (opt) {
	...
	function createCards(elem, linenum,data) {	
		var content = "<div class='layui-row layui-col-space10'>";
	...
	function createCard(elem, linenum, item, no) {
		var line = 12 / linenum;
		var card =
			'<div id=' + item.id + ' class="layui-col-md' + line + ' ew-datagrid-item" data-index="' + no+'" data-number="1"> <div class="project-list-item"> <img class="project-list-item-cover" src="' +item.image + '" onload="handleImgLoad(this);" params=\'' + item.params + '\'> <div class="project-list-item-body" style="padding: 10px;"> <div class="project-list-item-text layui-text" style="padding: 0px; height: auto; margin-bottom: 0;">' + item.cameraName + '</div> <div class="project-list-item-text layui-text" style="padding: 0px; height: auto; margin-bottom: 0;" id="status_' + item.id + '">' + item.wareName + '</div> <div class="project-list-item-text layui-text" style="padding: 0px; height: auto; margin-bottom: 0;">' + item.alarmTime + '</div> <div class="project-list-item-text layui-text" style="padding: 0px; height: auto; margin-bottom: 0; display: flex; flex-direction: row; justify-content: space-between;"><a href="javascript:void(0);" onclick="window.handleProcessed(\'' + item.id + '\');">人员已处理</a> <a href="javascript:void(0);" onclick="window.handleMisreport(\'' + item.id + '\');" style="color: #f43838;">人员误报</a> <a href="javascript:void(0);" onclick="window.handleDetail(\'' + item.id + '\');" style="color: #999;">详情</a></div> <div class="project-list-item-desc"> <div class="ew-head-list"></div> </div> </div > </div > </div > '
		return card;
	}
	...
	function initData(tempData, option) {
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			item.cameraName = dataList[i][option.request.cameraName];
			item.algorithmName = dataList[i][option.request.algorithmName];
			item.alarmTime = dataList[i][option.request.alarmTime];
			item.wareName = dataList[i][option.request.wareName];
			item.params = dataList[i][option.request.params];
			data.data.push(item);
		}
		return data;
    }
	...
	window.handleImgLoad = function(obj) {
		...
		for(var i = 0; i < len; i++) {
			...
			parentNode.append('<div style="position: absolute; border: 1px solid red; left: ' + (startX * ratio) + 'px; top: ' + (startY * ratio) + 'px; width: ' + (width * ratio) + 'px; height: ' + (height * ratio) + 'px;">' + type + '/' + confidence + '</div>');
		}
	}
