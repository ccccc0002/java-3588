...
<script>
    layui.use(['table', 'jquery', 'popup'], function() {
        ...
        // Support both legacy /smsphone and new /push/channel routes.
        let basePath = window.location.pathname.indexOf('/push/channel') === 0 ? '/push/channel' : '/smsphone';

        let cols = [[
            { title: '手机号码', field: 'phone' },
            { title: '操作', toolbar: '#table-actions', align: 'left', width: 120, fixed: 'right' }
        ]];
        ...
        window.removeData = function(obj) {
            ...
            layer.confirm('确定删除该推送号码吗？', {
                icon: 3,
                title: '提示'
            }, function(index) {
                layer.close(index);
                let loading = layer.load(2);
                $.post(basePath + '/delete', { id: obj.data.id }, function(res) {
                    layer.close(loading);
                    if (res.code === 0) {
