...
$deadLetterReplayBatchResumeTokenValid = ($deadLetterReplayBatchResumeTokenResp.code -ne 0) -and ($deadLetterReplayBatchResumeTokenTraceId -ne $null) -and ($deadLetterReplayBatchResumeTokenTraceId -ne "") -and ($deadLetterReplayBatchResumeTokenEnabled -is [bool]) -and ([bool]$deadLetterReplayBatchResumeTokenEnabled) -and ($deadLetterReplayBatchResumeTokenExpected -eq "resume-mismatch") -and ($deadLetterReplayBatchResumeTokenActual -ne $null) -and ($deadLetterReplayBatchResumeTokenActual -ne "") -and (([string]$deadLetterReplayBatchResumeTokenActual).Length -eq 64) -and ($deadLetterReplayBatchResumeTokenActual -ne "resume-mismatch")
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch(strict-resume-token-mismatch)" -Passed $deadLetterReplayBatchResumeTokenValid -Detail ("code={0}; trace_id={1}; strict_resume={2}; expected_resume={3}; actual_resume={4}" -f $deadLetterReplayBatchResumeTokenResp.code, $deadLetterReplayBatchResumeTokenTraceId, $deadLetterReplayBatchResumeTokenEnabled, $deadLetterReplayBatchResumeTokenExpected, $deadLetterReplayBatchResumeTokenActual)
