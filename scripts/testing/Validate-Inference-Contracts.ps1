...
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch" -Passed (($deadLetterReplayBatchResp.code -eq 0) -and ($deadLetterReplayBatchTraceId -ne $null) -and ($deadLetterReplayBatchTraceId -ne "") -and ($deadLetterReplayBatchResults -is [System.Collections.IList]) -and $deadLetterReplayBatchCountsConsistent) -Detail ("code={0}; trace_id={1}; selected={2}; processed={3}; success={4}; failed={5}; effective_limit={6}; max_limit={7}; truncated={8}; dry_run={9}; stop_on_error={10}; stopped_on_error={11}; dry_run_count={12}; failed_in_progress={13}; failed_exhausted={14}; failed_other={15}; list_size={16}; counts_consistent={17}" -f $deadLetterReplayBatchResp.code, $deadLetterReplayBatchTraceId, $deadLetterReplayBatchSelectedCount, $deadLetterReplayBatchProcessedCount, $deadLetterReplayBatchSuccessCount, $deadLetterReplayBatchFailedCount, $deadLetterReplayBatchEffectiveLimit, $deadLetterReplayBatchMaxLimit, $deadLetterReplayBatchTruncated, $deadLetterReplayBatchDryRun, $deadLetterReplayBatchStopOnError, $deadLetterReplayBatchStoppedOnError, $deadLetterReplayBatchDryRunCount, $deadLetterReplayBatchFailedReplayInProgressCount, $deadLetterReplayBatchFailedReplayExhaustedCount, $deadLetterReplayBatchFailedOtherCount, ($(if ($deadLetterReplayBatchResults -is [System.Collections.IList]) { $deadLetterReplayBatchResults.Count } else { -1 })), $deadLetterReplayBatchCountsConsistent)
...
$deadLetterReplayBatchByIdsValid = ($deadLetterReplayBatchByIdsResp.code -eq 0) -and ($deadLetterReplayBatchByIdsTraceId -ne $null) -and ($deadLetterReplayBatchByIdsTraceId -ne "") -and ($deadLetterReplayBatchByIdsSource -eq "explicit_ids") -and ($deadLetterReplayBatchByIdsResults -is [System.Collections.IList]) -and ($deadLetterReplayBatchByIdsProcessed -ne $null) -and ($deadLetterReplayBatchByIdsEffectiveLimit -ne $null) -and ($deadLetterReplayBatchByIdsMaxLimit -ne $null) -and ($deadLetterReplayBatchByIdsTruncated -is [bool]) -and ($deadLetterReplayBatchByIdsStopOnError -is [bool]) -and ($deadLetterReplayBatchByIdsStoppedOnError -is [bool]) -and ($deadLetterReplayBatchByIdsDryRunCount -ne $null) -and (([int]$deadLetterReplayBatchByIdsProcessed) -eq 2) -and (([int]$deadLetterReplayBatchByIdsDryRunCount) -eq 2) -and ($deadLetterReplayBatchByIdsResults.Count -eq 2) -and (([int]$deadLetterReplayBatchByIdsEffectiveLimit) -le ([int]$deadLetterReplayBatchByIdsMaxLimit)) -and (([bool]$deadLetterReplayBatchByIdsStopOnError) -eq $false) -and (([bool]$deadLetterReplayBatchByIdsStoppedOnError) -eq $false)
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch(by-ids,dry-run)" -Passed $deadLetterReplayBatchByIdsValid -Detail ("code={0}; trace_id={1}; source={2}; processed={3}; effective_limit={4}; max_limit={5}; truncated={6}; stop_on_error={7}; stopped_on_error={8}; dry_run_count={9}; list_size={10}" -f $deadLetterReplayBatchByIdsResp.code, $deadLetterReplayBatchByIdsTraceId, $deadLetterReplayBatchByIdsSource, $deadLetterReplayBatchByIdsProcessed, $deadLetterReplayBatchByIdsEffectiveLimit, $deadLetterReplayBatchByIdsMaxLimit, $deadLetterReplayBatchByIdsTruncated, $deadLetterReplayBatchByIdsStopOnError, $deadLetterReplayBatchByIdsStoppedOnError, $deadLetterReplayBatchByIdsDryRunCount, ($(if ($deadLetterReplayBatchByIdsResults -is [System.Collections.IList]) { $deadLetterReplayBatchByIdsResults.Count } else { -1 })))
...
$deadLetterReplayBatchBodyIdsValid = ($deadLetterReplayBatchBodyIdsResp.code -eq 0) -and ($deadLetterReplayBatchBodyIdsTraceId -ne $null) -and ($deadLetterReplayBatchBodyIdsTraceId -ne "") -and ($deadLetterReplayBatchBodyIdsSource -eq "explicit_ids") -and ($deadLetterReplayBatchBodyIdsResults -is [System.Collections.IList]) -and ($deadLetterReplayBatchBodyIdsProcessed -ne $null) -and ($deadLetterReplayBatchBodyIdsDryRunCount -ne $null) -and (([int]$deadLetterReplayBatchBodyIdsProcessed) -eq 2) -and (([int]$deadLetterReplayBatchBodyIdsDryRunCount) -eq 2) -and ($deadLetterReplayBatchBodyIdsResults.Count -eq 2)
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch(body-ids,dry-run)" -Passed $deadLetterReplayBatchBodyIdsValid -Detail ("code={0}; trace_id={1}; source={2}; processed={3}; dry_run_count={4}; list_size={5}" -f $deadLetterReplayBatchBodyIdsResp.code, $deadLetterReplayBatchBodyIdsTraceId, $deadLetterReplayBatchBodyIdsSource, $deadLetterReplayBatchBodyIdsProcessed, $deadLetterReplayBatchBodyIdsDryRunCount, ($(if ($deadLetterReplayBatchBodyIdsResults -is [System.Collections.IList]) { $deadLetterReplayBatchBodyIdsResults.Count } else { -1 })))
