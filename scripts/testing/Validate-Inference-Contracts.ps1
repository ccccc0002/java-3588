...
$checks += New-CheckResult -Api "/api/inference/route/batch" -Passed (($routeBatchResp.code -eq 0) -and ($routeBatchTraceId -ne $null) -and ($routeBatchTraceId -ne "") -and ($routeBatchGlobalBackend -ne $null) -and ($routeBatchGlobalBackend -ne "") -and ($routeBatchList -is [System.Collections.IList]) -and ($routeBatchList.Count -gt 0) -and ($routeBatchFirstCameraId -ne $null) -and (([long]$routeBatchFirstCameraId) -eq $CameraId) -and ($routeBatchFirstBackend -ne $null) -and ($routeBatchFirstBackend -ne "") -and ($routeBatchTruncated -is [bool]) -and (([int]$routeBatchMaxCameraIds) -eq 500) -and ($routeBatchDefaultFallbackUsed -is [bool]) -and ($routeBatchDefaultFallbackUsed -eq $false) -and ($routeBatchResolvedCameraCount -ne $null) -and (([int]$routeBatchResolvedCameraCount) -eq $routeBatchList.Count) -and ($routeBatchInputTokenCount -ne $null) -and (([int]$routeBatchInputTokenCount) -ge 0) -and ($routeBatchInvalidTokenCount -ne $null) -and (([int]$routeBatchInvalidTokenCount) -ge 0) -and ($routeBatchDuplicateFilteredCount -ne $null) -and (([int]$routeBatchDuplicateFilteredCount) -ge 0) -and ($routeBatchHitSources -is [System.Collections.IList]) -and ($routeBatchHitSources.Count -ge 1) -and ($routeBatchFirstHitSource -eq "body_camera_ids") -and ($routeBatchTruncatedSource -eq $null) -and (Is-ExpectedBackend -ActualBackend $routeBatchFirstBackend)) -Detail ("code={0}; trace_id={1}; global_backend_type={2}; first_camera_id={3}; first_backend_type={4}; truncated={5}; max_camera_ids={6}; default_fallback_used={7}; resolved_camera_count={8}; input_token_count={9}; invalid_token_count={10}; duplicate_filtered_count={11}; hit_sources={12}; truncated_source={13}; expected_backend={14}" -f $routeBatchResp.code, $routeBatchTraceId, $routeBatchGlobalBackend, $routeBatchFirstCameraId, $routeBatchFirstBackend, $routeBatchTruncated, $routeBatchMaxCameraIds, $routeBatchDefaultFallbackUsed, $routeBatchResolvedCameraCount, $routeBatchInputTokenCount, $routeBatchInvalidTokenCount, $routeBatchDuplicateFilteredCount, (($routeBatchHitSources -join ",")), $routeBatchTruncatedSource, $ExpectedBackendType)
