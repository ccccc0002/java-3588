...
$deadLetterReplayNotFoundErrorCode = Get-PropValue -Obj $deadLetterReplayNotFoundData -Name "error_code"
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay(not-found)" -Passed (($deadLetterReplayNotFoundResp.code -ne 0) -and ($deadLetterReplayNotFoundTraceId -ne $null) -and ($deadLetterReplayNotFoundTraceId -ne "") -and ($deadLetterReplayNotFoundId -ne $null) -and (([long]$deadLetterReplayNotFoundId) -eq -1) -and ($deadLetterReplayNotFoundErrorCode -eq "INFER_DL_REPLAY_NOT_FOUND")) -Detail ("code={0}; trace_id={1}; dead_letter_id={2}; error_code={3}" -f $deadLetterReplayNotFoundResp.code, $deadLetterReplayNotFoundTraceId, $deadLetterReplayNotFoundId, $deadLetterReplayNotFoundErrorCode)
...
$deadLetterReplayBatchExpectedResumeToken = Get-PropValue -Obj $deadLetterReplayBatchData -Name "expected_resume_token"
$deadLetterReplayBatchResumeToken = Get-PropValue -Obj $deadLetterReplayBatchData -Name "resume_token"
...
$deadLetterReplayBatchBackendTypeCounts = Get-PropValue -Obj $deadLetterReplayBatchData -Name "backend_type_counts"
$deadLetterReplayBatchFailureReasonCounts = Get-PropValue -Obj $deadLetterReplayBatchData -Name "failure_reason_counts"
$deadLetterReplayBatchReportStatusCounts = Get-PropValue -Obj $deadLetterReplayBatchData -Name "report_status_counts"
$deadLetterReplayBatchBackendTypeCountSize = Get-ObjectFieldCount -Obj $deadLetterReplayBatchBackendTypeCounts
$deadLetterReplayBatchFailureReasonCountSize = Get-ObjectFieldCount -Obj $deadLetterReplayBatchFailureReasonCounts
$deadLetterReplayBatchReportStatusCountSize = Get-ObjectFieldCount -Obj $deadLetterReplayBatchReportStatusCounts
...
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch" -Passed (($deadLetterReplayBatchResp.code -eq 0) -and ($deadLetterReplayBatchTraceId -ne $null) -and ($deadLetterReplayBatchTraceId -ne "") -and ($deadLetterReplayBatchResults -is [System.Collections.IList]) -and $deadLetterReplayBatchCountsConsistent) -Detail ("code={0}; trace_id={1}; selected={2}; requested_offset={3}; effective_offset={4}; next_offset={5}; has_more={6}; total_selected={7}; strict_resume={8}; expected_total={9}; actual_total={10}; expected_window={11}; actual_window={12}; expected_resume={13}; resume_token={14}; processed={15}; success={16}; failed={17}; effective_limit={18}; max_limit={19}; truncated={20}; dry_run={21}; stop_on_error={22}; stopped_on_error={23}; dry_run_count={24}; remaining_count={25}; success_ids={26}; failed_ids={27}; dry_run_ids={28}; failed_in_progress={29}; failed_exhausted={30}; failed_other={31}; backend_type_count_size={32}; failure_reason_count_size={33}; report_status_count_size={34}; list_size={35}; counts_consistent={36}" -f $deadLetterReplayBatchResp.code, $deadLetterReplayBatchTraceId, $deadLetterReplayBatchSelectedCount, $deadLetterReplayBatchRequestedOffset, $deadLetterReplayBatchEffectiveOffset, $deadLetterReplayBatchNextOffset, $deadLetterReplayBatchHasMore, $deadLetterReplayBatchTotalSelectedCount, $deadLetterReplayBatchStrictResume, $deadLetterReplayBatchExpectedTotal, $deadLetterReplayBatchActualTotal, $deadLetterReplayBatchExpectedWindowFingerprint, $deadLetterReplayBatchActualWindowFingerprint, $deadLetterReplayBatchExpectedResumeToken, $deadLetterReplayBatchResumeToken, $deadLetterReplayBatchProcessedCount, $deadLetterReplayBatchSuccessCount, $deadLetterReplayBatchFailedCount, $deadLetterReplayBatchEffectiveLimit, $deadLetterReplayBatchMaxLimit, $deadLetterReplayBatchTruncated, $deadLetterReplayBatchDryRun, $deadLetterReplayBatchStopOnError, $deadLetterReplayBatchStoppedOnError, $deadLetterReplayBatchDryRunCount, $deadLetterReplayBatchRemainingCount, ($(if ($deadLetterReplayBatchSuccessIds -is [System.Collections.IList]) { $deadLetterReplayBatchSuccessIds.Count } else { -1 })), ($(if ($deadLetterReplayBatchFailedIds -is [System.Collections.IList]) { $deadLetterReplayBatchFailedIds.Count } else { -1 })), ($(if ($deadLetterReplayBatchDryRunIds -is [System.Collections.IList]) { $deadLetterReplayBatchDryRunIds.Count } else { -1 })), $deadLetterReplayBatchFailedReplayInProgressCount, $deadLetterReplayBatchFailedReplayExhaustedCount, $deadLetterReplayBatchFailedOtherCount, $deadLetterReplayBatchBackendTypeCountSize, $deadLetterReplayBatchFailureReasonCountSize, $deadLetterReplayBatchReportStatusCountSize, ($(if ($deadLetterReplayBatchResults -is [System.Collections.IList]) { $deadLetterReplayBatchResults.Count } else { -1 })), $deadLetterReplayBatchCountsConsistent)
...
$deadLetterReplayBatchStrictResumeMissingReq = @{
    strict_resume = 1
    dead_letter_ids = @(801, 802)
    dry_run = 1
}
$deadLetterReplayBatchStrictResumeMissingResp = Invoke-ApiPostJson -Path "/api/inference/dead-letter/replay/batch?limit=2" -BodyObj $deadLetterReplayBatchStrictResumeMissingReq
$deadLetterReplayBatchStrictResumeMissingData = Get-PropValue -Obj $deadLetterReplayBatchStrictResumeMissingResp -Name "data"
$deadLetterReplayBatchStrictResumeMissingTraceId = Get-PropValue -Obj $deadLetterReplayBatchStrictResumeMissingData -Name "trace_id"
$deadLetterReplayBatchStrictResumeMissingEnabled = Get-PropValue -Obj $deadLetterReplayBatchStrictResumeMissingData -Name "strict_resume"
$deadLetterReplayBatchStrictResumeMissingActual = Get-PropValue -Obj $deadLetterReplayBatchStrictResumeMissingData -Name "actual_total_selected_count"
$deadLetterReplayBatchStrictResumeMissingErrorCode = Get-PropValue -Obj $deadLetterReplayBatchStrictResumeMissingData -Name "error_code"
$deadLetterReplayBatchStrictResumeMissingValid = ($deadLetterReplayBatchStrictResumeMissingResp.code -ne 0) -and ($deadLetterReplayBatchStrictResumeMissingTraceId -ne $null) -and ($deadLetterReplayBatchStrictResumeMissingTraceId -ne "") -and ($deadLetterReplayBatchStrictResumeMissingEnabled -is [bool]) -and ([bool]$deadLetterReplayBatchStrictResumeMissingEnabled) -and ($deadLetterReplayBatchStrictResumeMissingActual -ne $null) -and (([int]$deadLetterReplayBatchStrictResumeMissingActual) -eq 2) -and ($deadLetterReplayBatchStrictResumeMissingErrorCode -eq "INFER_DL_REPLAY_BATCH_STRICT_RESUME_EXPECTED_TOTAL_REQUIRED")
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch(strict-resume-missing-expected-total)" -Passed $deadLetterReplayBatchStrictResumeMissingValid -Detail ("code={0}; trace_id={1}; strict_resume={2}; actual_total={3}; error_code={4}" -f $deadLetterReplayBatchStrictResumeMissingResp.code, $deadLetterReplayBatchStrictResumeMissingTraceId, $deadLetterReplayBatchStrictResumeMissingEnabled, $deadLetterReplayBatchStrictResumeMissingActual, $deadLetterReplayBatchStrictResumeMissingErrorCode)
...
$deadLetterReplayBatchStrictResumeErrorCode = Get-PropValue -Obj $deadLetterReplayBatchStrictResumeData -Name "error_code"
$deadLetterReplayBatchStrictResumeValid = ($deadLetterReplayBatchStrictResumeResp.code -ne 0) -and ($deadLetterReplayBatchStrictResumeTraceId -ne $null) -and ($deadLetterReplayBatchStrictResumeTraceId -ne "") -and ($deadLetterReplayBatchStrictResumeEnabled -is [bool]) -and ([bool]$deadLetterReplayBatchStrictResumeEnabled) -and ($deadLetterReplayBatchStrictResumeExpected -ne $null) -and (([int]$deadLetterReplayBatchStrictResumeExpected) -eq 999) -and ($deadLetterReplayBatchStrictResumeActual -ne $null) -and (([int]$deadLetterReplayBatchStrictResumeActual) -eq 2) -and ($deadLetterReplayBatchStrictResumeErrorCode -eq "INFER_DL_REPLAY_BATCH_STRICT_RESUME_TOTAL_SELECTED_COUNT_MISMATCH")
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch(strict-resume-mismatch)" -Passed $deadLetterReplayBatchStrictResumeValid -Detail ("code={0}; trace_id={1}; strict_resume={2}; expected_total={3}; actual_total={4}; error_code={5}" -f $deadLetterReplayBatchStrictResumeResp.code, $deadLetterReplayBatchStrictResumeTraceId, $deadLetterReplayBatchStrictResumeEnabled, $deadLetterReplayBatchStrictResumeExpected, $deadLetterReplayBatchStrictResumeActual, $deadLetterReplayBatchStrictResumeErrorCode)
...
$deadLetterReplayBatchFingerprintErrorCode = Get-PropValue -Obj $deadLetterReplayBatchFingerprintData -Name "error_code"
$deadLetterReplayBatchFingerprintValid = ($deadLetterReplayBatchFingerprintResp.code -ne 0) -and ($deadLetterReplayBatchFingerprintTraceId -ne $null) -and ($deadLetterReplayBatchFingerprintTraceId -ne "") -and ($deadLetterReplayBatchFingerprintEnabled -is [bool]) -and ([bool]$deadLetterReplayBatchFingerprintEnabled) -and ($deadLetterReplayBatchFingerprintExpected -eq "deadbeef") -and ($deadLetterReplayBatchFingerprintActual -ne $null) -and ($deadLetterReplayBatchFingerprintActual -ne "") -and (([string]$deadLetterReplayBatchFingerprintActual).Length -eq 64) -and ($deadLetterReplayBatchFingerprintActual -ne "deadbeef") -and ($deadLetterReplayBatchFingerprintErrorCode -eq "INFER_DL_REPLAY_BATCH_STRICT_RESUME_WINDOW_FINGERPRINT_MISMATCH")
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch(strict-resume-fingerprint-mismatch)" -Passed $deadLetterReplayBatchFingerprintValid -Detail ("code={0}; trace_id={1}; strict_resume={2}; expected_window={3}; actual_window={4}; error_code={5}" -f $deadLetterReplayBatchFingerprintResp.code, $deadLetterReplayBatchFingerprintTraceId, $deadLetterReplayBatchFingerprintEnabled, $deadLetterReplayBatchFingerprintExpected, $deadLetterReplayBatchFingerprintActual, $deadLetterReplayBatchFingerprintErrorCode)

$deadLetterReplayBatchResumeTokenReq = @{
    strict_resume = 1
    expected_total_selected_count = 2
    expected_resume_token = "resume-mismatch"
    dead_letter_ids = @(841, 842)
    dry_run = 1
}
$deadLetterReplayBatchResumeTokenResp = Invoke-ApiPostJson -Path "/api/inference/dead-letter/replay/batch?limit=2" -BodyObj $deadLetterReplayBatchResumeTokenReq
$deadLetterReplayBatchResumeTokenData = Get-PropValue -Obj $deadLetterReplayBatchResumeTokenResp -Name "data"
$deadLetterReplayBatchResumeTokenTraceId = Get-PropValue -Obj $deadLetterReplayBatchResumeTokenData -Name "trace_id"
$deadLetterReplayBatchResumeTokenEnabled = Get-PropValue -Obj $deadLetterReplayBatchResumeTokenData -Name "strict_resume"
$deadLetterReplayBatchResumeTokenExpected = Get-PropValue -Obj $deadLetterReplayBatchResumeTokenData -Name "expected_resume_token"
$deadLetterReplayBatchResumeTokenActual = Get-PropValue -Obj $deadLetterReplayBatchResumeTokenData -Name "actual_resume_token"
$deadLetterReplayBatchResumeTokenErrorCode = Get-PropValue -Obj $deadLetterReplayBatchResumeTokenData -Name "error_code"
$deadLetterReplayBatchResumeTokenValid = ($deadLetterReplayBatchResumeTokenResp.code -ne 0) -and ($deadLetterReplayBatchResumeTokenTraceId -ne $null) -and ($deadLetterReplayBatchResumeTokenTraceId -ne "") -and ($deadLetterReplayBatchResumeTokenEnabled -is [bool]) -and ([bool]$deadLetterReplayBatchResumeTokenEnabled) -and ($deadLetterReplayBatchResumeTokenExpected -eq "resume-mismatch") -and ($deadLetterReplayBatchResumeTokenActual -ne $null) -and ($deadLetterReplayBatchResumeTokenActual -ne "") -and (([string]$deadLetterReplayBatchResumeTokenActual).Length -eq 64) -and ($deadLetterReplayBatchResumeTokenActual -ne "resume-mismatch") -and ($deadLetterReplayBatchResumeTokenErrorCode -eq "INFER_DL_REPLAY_BATCH_STRICT_RESUME_TOKEN_MISMATCH")
$checks += New-CheckResult -Api "/api/inference/dead-letter/replay/batch(strict-resume-token-mismatch)" -Passed $deadLetterReplayBatchResumeTokenValid -Detail ("code={0}; trace_id={1}; strict_resume={2}; expected_resume={3}; actual_resume={4}; error_code={5}" -f $deadLetterReplayBatchResumeTokenResp.code, $deadLetterReplayBatchResumeTokenTraceId, $deadLetterReplayBatchResumeTokenEnabled, $deadLetterReplayBatchResumeTokenExpected, $deadLetterReplayBatchResumeTokenActual, $deadLetterReplayBatchResumeTokenErrorCode)
