...
class JavaAppControllerTests(unittest.TestCase):
    def test_start_app_uses_repo_start_script_and_returns_started(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(java_app_ctl, 'fetch_health', side_effect=[{'http_status': 0, 'payload': {}}, {'http_status': 200, 'payload': {'code': 0}}]), \
    ...
    def test_start_app_returns_already_running_when_health_is_up(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(java_app_ctl, 'fetch_health', return_value={'http_status': 200, 'payload': {'code': 0}}), \
    ...
    def test_stop_app_uses_pkill_pattern(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = pathlib.Path(temp_dir)
            config = java_app_ctl.AppControllerConfig(
                repo_root=repo_root,
            ...
            with mock.patch.object(java_app_ctl.subprocess, 'run', return_value=mock.Mock(returncode=0, stdout='', stderr='')) as run_mock:
                result = java_app_ctl.stop_app(config)

            self.assertEqual('stopped', result['status'])
            run_mock.assert_called_once_with(['pkill', '-f', 'java-rk3588-0.0.1-SNAPSHOT.jar.*18082'], capture_output=True, text=True)
    ...
    def test_status_app_reports_down_when_health_fails(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(java_app_ctl, 'fetch_health', return_value={'http_status': 0, 'payload': {'message': 'connection refused'}}):
