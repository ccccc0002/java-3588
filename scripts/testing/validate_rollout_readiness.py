...
def main(argv: Optional[Sequence[str]] = None, client: Optional[Any] = None) -> int:
    ...
    client_impl = client if client is not None else (
        DryRunClient(args.primary_camera_id, args.secondary_camera_id) if args.dry_run else ApiClient(args.base_url, args.timeout_sec, args.cookie, args.auth_header_name, args.auth_header_value)
    )
